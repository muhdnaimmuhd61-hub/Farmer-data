...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        # Example: add all states + some LGAs (expand as needed)
        all_locations = {
...
lang_dict = {
    "en": {"title":"Farmers Registration","state":"State","lga":"LGA","name":"Full Name","crop":"Crop Type",
           "location":"Farming Location","phone":"Phone","submit":"Submit","dashboard":"Dashboard",
           "filter_state":"Filter by State","filter_lga":"Filter by LGA","filter_name":"Filter by Name",
           "weather_alert":"Weather/Flood Alert","recommended_crop":"Recommended Crop"},
    "ha": {"title":"Rijistar Manoma","state":"Jihar","lga":"Karamar Hukuma (LGA)","name":"Cikakken Suna",
           "crop":"Irin Amfanin Gona","location":"Wurin Noma","phone":"Lambar Waya","submit":"Tura",
           "dashboard":"Dashboard","filter_state":"Filter Jihar","filter_lga":"Filter LGA","filter_name":"Filter Suna",
           "weather_alert":"Yanayi / Ambaliya","recommended_crop":"Amfanin gona da ya dace"},
    "yo": {"title":"Ìforúkọsílẹ̀ Ọ̀gbìn","state":"Ìpínlẹ̀","lga":"Agbegbe","name":"Orúkọ",
           "crop":"Irú Ọ̀gbìn","location":"Àgbègbè Ọ̀gbìn","phone":"Fóònù","submit":"Firanṣẹ",
           "dashboard":"Dashboard","filter_state":"Ṣàlẹ̀ Jihà","filter_lga":"Ṣàlẹ̀ Agbegbe","filter_name":"Ṣàlẹ̀ Orúkọ",
           "weather_alert":"Ìkìlọ̀ Òjò / Àdánwò","recommended_crop":"Ọ̀gbìn Tó Dára"},
    "ig": {"title":"Ndebanye Ndị Ọgwụ","state":"Steeti","lga":"LGA","name":"Aha","crop":"Ụdị Ọgwụ",
           "location":"Ebe A na-akụ","phone":"Ekwentị","submit":"Zipu","dashboard":"Dashboard",
           "filter_state":"Nyocha Steeti","filter_lga":"Nyocha LGA","filter_name":"Nyocha Aha",
           "weather_alert":"Ilu / Miri","recommended_crop":"Ụdị Ọgwụ Dị Mma"}
}
...
img{max-width:100px;border-radius:5px;margin:5px;}
</style>
</head>
<body>
<div class='card'>
...
<label>{{strings['state']}}</label><select name='state' id='state' required onchange='populateLGAs()'>
<option value=''>{{strings['state']}}</option>
{% for s in states %}<option value='{{s}}'>{{s}}</option>{% endfor %}
</select>
<label>{{strings['lga']}}</label><select name='lga' id='lga' required><option value=''>{{strings['lga']}}</option></select>
<label>{{strings['location']}}</label><input type='text' name='location' required>
...
{% if success %}<p style='color:green;'>Data saved successfully!</p>{% endif %}
<p><a href='/dashboard?lang={{lang}}'>{{strings['dashboard']}}</a> | <a href='/download'>Download CSV</a></p>
...
function populateLGAs(){
    ...
    lgaSelect.innerHTML='<option value="">{{strings["lga"]}}</option>';
    if(state in lga_data){
        lga_data[state].forEach(lga=>{
            let opt=document.createElement('option');
            opt.value=lga; opt.innerHTML=lga;
...
<style>body{font-family:Arial;background:#f5f5f5;padding:20px;} img{max-width:80px;border-radius:5px;}</style>
</head>
<body>
<h2>{{strings['dashboard']}}</h2>
<form method='GET'>
<label>{{strings['filter_state']}}</label>
...
<label>{{strings['filter_lga']}}</label>
...
<label>{{strings['filter_name']}}</label><input type='text' name='filter_name' value='{{selected_name}}' onchange='this.form.submit()'>
</form>
<table border='1' cellpadding='5' cellspacing='0'>
<tr><th>{{strings['name']}}</th><th>{{strings['state']}}</th><th>{{strings['lga']}}</th><th>{{strings['crop']}}</th>
<th>{{strings['location']}}</th><th>{{strings['phone']}}</th><th>{{strings['weather_alert']}}</th>
<th>{{strings['recommended_crop']}}</th><th>Photo</th></tr>
{% for f in farmers %}
<tr>
<td>{{f['name']}}</td><td>{{f['state']}}</td><td>{{f['lga']}}</td><td>{{f['crop']}}</td>
<td>{{f['location']}}</td><td>{{f['phone']}}</td><td>{{f['alert']}}</td>
<td>{{f['recommendation']}}</td>
<td>{% if f['photo_path'] %}<img src='/{{f['photo_path']}}'>{% endif %}</td>
</tr>
{% endfor %}
</table>
<p><a href='/?lang={{lang}}'>Back to Registration</a></p>
</body>
</html>
"""
...
def home():
    ...
    lga_json = {}
    for s,l in c.fetchall():
        lga_json.setdefault(s,[]).append(l)
    conn.close()
...
def dashboard():
    ...
    selected_name = request.args.get('filter_name','')
    ...
    for f in farmers_raw:
        ...
        farmers.append({"name":f[0],"state":f[1],"lga":f[2],"crop":f[3],
                        "location":f[4],"phone":f[5],"photo_path=f[6].replace('\\\\','/')","alert":alert,"recommendation":rec})
    ...
    return render_template_string(dashboard_template, lang=lang, strings=strings, farmers=farmers,
                                  states=states, lgas=lgas, selected_state=selected_state,
                                  selected_lga=selected_lga, selected_name=selected_name)

@app.route('/download')
def download_csv():
    conn = sqlite3.connect('farmers.db')
    c = conn.cursor()
    c.execute("SELECT name,state,lga,location,crop,phone,photo_path FROM farmers")
    rows = c.fetchall()
    conn.close()
    path = 'farmers_data.csv'
    with open(path,'w',newline='',encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Name","State","LGA","Location","Crop","Phone","Photo"])
